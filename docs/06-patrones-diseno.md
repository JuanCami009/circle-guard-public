...
```bash
# Compilar todos los servicios modificados
...
### Smoke manual Circuit Breaker

```bash
# 1. Levantar auth-service con identity-service apagado
docker compose up auth-service -d

# 2. Hacer 5 requests a /auth/login → ver logs: retry x2 → CB OPEN
# 3. Tras 30s, hacer 1 request → HALF_OPEN → identity arriba → CLOSED
```

### Smoke manual Feature Toggle

```bash
# Mock (por defecto)
PUSH_REAL_DELIVERY=false docker compose up notification-service -d
# Logs: [MOCK PUSH] To: ...

# Real
PUSH_REAL_DELIVERY=true GOTIFY_TOKEN=mi-token docker compose up notification-service -d
```

### Smoke manual Cache-Aside

```bash
# Levantar gateway-service + Redis, validar mismo token 10 veces
# Redis debe recibir exactamente 1 GET, las 9 restantes sirve Caffeine
```
