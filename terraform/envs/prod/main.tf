...
module "secrets" {
  ...
  secrets = {
    jwt = {
      ...
      value       = jsonencode({ jwt_secret = "prod-jwt-secret-key-must-be-32chars!", qr_secret = "prod-qr-secret-key-1234567890abc" })
    }
    postgres = {
    ...
    twilio = {
      description = "Twilio SMS credentials (prod placeholders)"
      value       = jsonencode({ account_sid = "prod_placeholder", auth_token = "prod_placeholder" })
    }
    vault = {
      description = "Identity service vault secrets"
      value       = jsonencode({ secret = "prod-vault-secret-key-32-chars-!!", salt = "deadbeef", hash_salt = "12345678" })
    }
    ldap = {
  ...
  services = {
    ...
    "identity-service" = {
      ...
      extra_env = {
        ...
        VAULT_SECRET          = "prod-vault-secret-key-32-chars-!!"
        VAULT_SALT            = "deadbeef"
